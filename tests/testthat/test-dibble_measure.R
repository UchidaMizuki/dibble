test_that("dibble_measure", {
  x <- array(1:6, 2:3)
  dim_names <- list(axis1 = letters[1:2], axis2 = letters[1:3])

  x1 <- dibble_measure(x, dim_names)

  dimnames(x) <- dim_names

  x2 <- dibble_measure(x1)
  x3 <- dibble_measure(x2)
...
test_that("is_dibble_measure", {
  expect_false(is_dibble_measure(table1_dibble))
  expect_true(is_dibble_measure(table1_dibble$cases))
})

test_that("as.table.dibble_measure", {
  x <- array(1:6, 2:3,
             dimnames = list(axis1 = letters[1:2], axis2 = letters[1:3]))
  x <- as.table(x)

  x1 <- as_dibble_measure(x)
  x1 <- as.table(x1)

  expect_equal(x, x1)
})

test_that("dimnames.dibble_measure", {
...
test_that("nrow-ncol-row-colnames.dibble_measure", {
  x <- table1_dibble$cases
...
test_that("slice.dibble_measure", {
...
test_that("select.dibble_measure", {
  x <- table1_dibble$population
  x <- select(x, year)
...
test_that("relocate.dibble_measure", {
  x <- table1_dibble$population
  x <- relocate(x, year)
...
test_that("rename.dibble_measure", {
  x <- table1_dibble$population
  x <- rename(x,
              country1 = country,
              year1 = year)
...
test_that("print.dibble_measure", {
